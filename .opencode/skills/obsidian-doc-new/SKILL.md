...
description: 新規ドキュメントを採番して作成しテンプレ適用とリンク整備を行う。使う場面: 新規要求/設計/テスト文書の追加、ID採番、初期Frontmatter作成が必要な依頼時。
metadata:
...
- ドキュメントID採番と配置先決定を自動化したい依頼

## When NOT to use me
- 既存ドキュメントの部分修正のみ（`obsidian-doc-change` を使う）
- 新規作成ではなく影響確認中心（`docops-orchestrator` を使う）
...
2. **採番**
   ...
   - 例: `python .opencode/skills/obsidian-doc-new/scripts/next_id.py RQ-FR --json`
...
## Output contract
- 新規作成されたMarkdownファイル（`id` とファイル名一致）
- 初期Frontmatter（必須キー埋め）と `## 変更履歴`
- 必要時に `reports/doc_check.md`
